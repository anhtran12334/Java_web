								...
								<tr data-id="${saleOrder.id }">
									...
									<td><a class="btn btn-primary edit-product"
										...
										role="button" style="font-size: 13px;"><i
											class="fas fa-edit"></i> Detail </a> <a class="btn btn-success "
										href="#" role="button" style="font-size: 13px;"
										onclick="receiveSaleOrder(${saleOrder.id })"><i
											class="fas fa-check-circle"></i> </a> <a class="btn btn-danger "
										href="#" role="button" style="font-size: 13px;"
										onclick="deleteSaleOrder(${saleOrder.id },${saleOrder.user.id })"><i
...
<script type="text/javascript">
	...
	function receiveSaleOrder(id){
		 ...
		 const isConfirm = window.confirm('Bạn có chắc muốn tiếp nhận đơn hàng này không ?')
		if(!isConfirm) return
		let data = {
				id: id,// lay theo id
		};
		$.ajax({
			url : "/receiveSaleOrder/" +id,
			type : "post",
			contentType : "application/json",
			data : JSON.stringify(data),
			dataType: "json", // kieu du lieu tra ve la json
